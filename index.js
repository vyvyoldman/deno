/**
 * Deno Version of the Argo/Nezha Script
 * * 运行命令:
...
const AUTO_ACCESS = Deno.env.get("AUTO_ACCESS") === "true"; // Deno env returns string
...
function getSystemArchitecture() {
  const arch = Deno.build.arch; // "x86_64" or "aarch64"
  if (arch === "aarch64") {
    return "arm";
  } else {
    return "amd";
...
function getFilesForArchitecture(architecture: string) {
  let baseFiles: { fileName: string; fileUrl: string }[] = [];
  if (architecture === "arm") {
    baseFiles = [
  ...
  if (ARGO_AUTH && ARGO_DOMAIN) {
    ...
    try {
      ...
      if (argoDomains.length > 0) {
        ...
        // Deno doesn't have easy pkill, assuming bot crashes/restarts or just spawn new
        // Killing logic is complex cross-platform in Deno without external tools.
        // We will just try to spawn a new one as per logic flow.
        ...
        try {
          ...
          console.log(`${botName} is running`);
          await new Promise((resolve) => setTimeout(resolve, 3000));
          await extractDomains(); // recursion
        } catch (error) {
          console.error(`Error executing command: ${error}`);
        }
      }
...
async function AddVisitTask() {
  ...
  try {
    ...
    console.error(
      `Add automatic access task faild: ${(error as Error).message}`
    );
  }
}
...
async function startserver() {
  try {
    ...
    cleanFiles(); // Run in parallel timer
    await AddVisitTask();
  } catch (error) {
